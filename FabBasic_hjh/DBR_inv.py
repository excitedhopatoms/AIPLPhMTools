...
def SGDBRFromCsvOffset(
    ...
    sampling_mask = (indices % PeriodSampled) > (PeriodSampled - NumSampled - 1)  # 为适应0-based索引进行调整
...
# %% DBRFromCsv: 从 CSV 文件创建 DBR
@gf.cell
def VSGDBRFromCsvOffset(
        CSVName: str = "D:/Mask Download/Temp202311_LN_ZJ/单_D01.5e-25_k0.5_1500-1600.csv",  # CSV 文件路径
        WidthHeat: float = 4,  # 加热器宽度 (µm)
        WidthRoute: float = 10,  # 加热器路由宽度 (µm)
        PeriodSampled: list | np.ndarray = None,  # ← 改成列表/数组
        NumSampled: list | np.ndarray = None,  # ← 改成列表/数组
        Offset: float = 0.5,
        IsHeat: bool = False,  # 是否包含加热器
        Wcol=[1, 3],
        Lcol=[2, 4],
    ...
    参数:
        CSVName (str): CSV文件的完整路径。
        Offset (float): 应用于每周期中较宽部分的宽度偏移量。
        PeriodSampled (int): 一个采样超周期（super-period）内的总周期数。
        NumSampled (int): 在一个超周期末尾，宽度需要被修改的周期数。
        IsHeat (bool): 如果为True，则在DBR上方添加一个加热器。
        WidthHeat (float): 加热条的宽度。
        WidthRoute (float): 加热器引出金属的宽度。
    ...
    # C. **多组采样** —— 依次叠加掩码
    sampling_mask = np.zeros(num_periods, dtype=bool)
    for P, N in zip(PeriodSampled, NumSampled):
        # 每组规则：超周期 P 内最后 N 个周期被采样
        mask = (indices % P) >= (P - N)
        sampling_mask |= mask  # 任意一组命中即采样
    w0_after_sampling = np.where(sampling_mask, w1_after_offset, w0_after_offset)
...
def EstrDBRFromCsvOffset(
        CSVName: str = "D:/Mask Download/Temp202311_LN_ZJ/",  # CSV 文件路径
...
def make_dot_dbr_structure(
        WidthMidWG: float = 0.8,
        HoleRadius: float = 0.15,
        HoleOffset: float = 0.5,
        LengthPeriod: float = 1.1,
        DutyCycle: float = 0.5,
        NumHolesPerSide: int = 1,
        oplayer: LayerSpec = LAYER.WG,
) -> Component:
    """
    创建圆孔型 DBR 单周期结构：中心波导 + 两侧均匀分布的圆孔。

    在正相结构中，圆孔以 WG 层多边形表示；经 EDBRStrRep_InvertTone
    反相后，圆孔变为背景中的真实孔洞，中心波导变为刻蚀槽。

    Args:
        WidthMidWG: 中心波导宽度 (um)
        HoleRadius: 圆孔半径 (um)
        HoleOffset: 圆孔中心距波导中心的 y 方向偏移 (um)
        LengthPeriod: 单周期长度 (um)
        DutyCycle: 有孔区域占周期比例 (0~1)
        NumHolesPerSide: 每侧每周期圆孔数量
        oplayer: 图层

    Returns:
        单周期圆孔 DBR 结构，带 o1/o2 端口
    """
    c = gf.Component()

    wg = gf.c.straight(
        length=LengthPeriod,
        cross_section=gf.cross_section.strip(width=WidthMidWG, layer=oplayer),
    )
    c << wg

    hole_start = LengthPeriod * (1 - DutyCycle)
    hole_length = LengthPeriod * DutyCycle

    if NumHolesPerSide > 0 and hole_length > 1e-9:
        hole_spacing = hole_length / (NumHolesPerSide + 1)
        for i in range(1, NumHolesPerSide + 1):
            x_center = hole_start + i * hole_spacing
            for y_sign in [-1, 1]:
                hole = gf.components.circle(radius=HoleRadius, layer=oplayer)
                hole_ref = c << hole
                hole_ref.move((x_center, y_sign * HoleOffset))

    c.add_port('o1', center=(0, 0), width=WidthMidWG, orientation=180, layer=oplayer)
    c.add_port('o2', center=(LengthPeriod, 0), width=WidthMidWG, orientation=0, layer=oplayer)
...
def EDBRStrRep(
    ...
    if Structure is not None:
        if 'o1' in Structure.ports and 'o2' in Structure.ports:
            ...
            LengthPeriod = ybbox[1][0] - ybbox[0][0]
...
def EDBRStrRep_InvertTone(
        ...
        vialayer: LayerSpec = LAYER.VIA,
        UseParallel: bool = True,
        TrenchWidth: float = 4.0  # 正胶版图外扩的背景挖槽宽度 (µm)
) -> Component:
    """
    ...
    # ---------------------------------------------------------
    # 创建单周期大小的背景矩形
    bg = gf.components.rectangle(size=(LengthPeriod, TrenchWidth), layer=oplayer)
    temp_bg_comp = gf.Component()
    bg_ref = temp_bg_comp << bg
    bg_ref.move((0, -TrenchWidth / 2))
...
# %% 导出所有函数
__all__ = ['DBR', 'DBRFromCsv', 'DBRFromCsvOffset', 'SGDBRFromCsvOffset', 'EstrDBRFromCsvOffset', 'EDBRStrRep', 'VSGDBRFromCsvOffset', 'EDBRFromCsv', 'EDBRStrRep_InvertTone', 'EDBRStrRep_InvertTone_Full', 'make_flush_inverted_cell', 'Complete_Inverted_Device']
