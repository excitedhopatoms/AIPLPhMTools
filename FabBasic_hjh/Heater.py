...
def ViaArrayParallel(
    ...
    参数说明见原始文档。
    """
    CompCopy = gf.Component()
    CompCopy << CompEn
    via = gf.Component()
    B = gf.Component()
    if Spacing:
        Period = Spacing+WidthVia

    fill = gf.c.rectangle(layer=vialayer, size=(WidthVia, WidthVia))
    CompCopy.fill(
        fill_cell=fill,
        fill_layers=[(arraylayer, -Enclosure)],
        exclude_layers=[],
        x_space=Period,
        y_space=Period,
    )
    via_array=GetFromLayer(CompCopy,arraylayer)
    return via_array
